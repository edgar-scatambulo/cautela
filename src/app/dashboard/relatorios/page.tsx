            ...
            <div className="space-y-4">
              {filteredLoans.map((loan, index) => {
                ...
                <div key={loan.id} className="p-6">
                  <div className="flex items-start justify-between">
                    <div className="flex-grow space-y-1.5">
                        ...
                        <div className="flex items-center text-sm text-muted-foreground">
                          ...
                          <span className="text-foreground mr-1">Cautela:</span>
                          {format(parse(loan.loanDate, 'yyyy-MM-dd', new Date()), "dd/MM/yy", { locale: ptBR })} às {loan.loanTime}
                        </div>
                        {radioOperator && (
                            <div className="flex items-center text-sm text-muted-foreground">
                                <User className="h-4 w-4 mr-2" />
                                <span className="text-foreground mr-1">Entregue por:</span>
                                {radioOperator.name}
                            </div>
                        )}
                        {loan.status === LoanStatus.DEVOLVIDO && loan.actualReturnDate && loan.actualReturnTime && (
                            <div className="flex items-center text-sm text-muted-foreground">
                            <CalendarDays className="h-4 w-4 mr-2" /> 
                             <span className="text-foreground mr-1">Devolução:</span>
                            {format(parse(loan.actualReturnDate, 'yyyy-MM-dd', new Date()), "dd/MM/yy", { locale: ptBR })} às {loan.actualReturnTime}
                            </div>
                        )}
                        {returnedByUser && (
                        <div className="flex items-center text-sm text-muted-foreground">
                            <User className="h-4 w-4 mr-2" />
                             <span className="text-foreground mr-1">Recebido por:</span>
                            {returnedByUser.name}
                        </div>
                        )}
                    </div>
                  ...
                  {index < filteredLoans.length - 1 && <Separator className="mt-6"/>}
                </div>
              )})}
            </div>
          </CardContent>
