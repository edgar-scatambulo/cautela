...
import type { Equipment, PoliceOfficer } from '@/lib/types';
...
export default function DashboardPage() {
  const { currentUser, equipments, officers, loans, addLoan, addEquipment, updateLoanStatus } = useStore();
                    ...
                    <SelectContent>
                      {activeLoansList.map(loan => (
                        <SelectItem key={loan.id} value={loan.id}>
                          Cautela #{loan.id.substring(0, 6)} (Policial: {getOfficerName(loan.officerId, officers)}) Data: {format(parseISO(loan.loanDate), "dd/MM/yy", { locale: ptBR })}
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
